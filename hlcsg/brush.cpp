...
find_plane:
	for (; returnval < g_nummapplanes; returnval++)
	{
			...
			-NORMAL_EPSILON < (t = normal[2] - g_mapplanes[returnval].normal[2]) && t < NORMAL_EPSILON)
		{
			...
			t = (origin[0] - g_mapplanes[returnval].origin[0]) * normal[0] + (origin[1] - g_mapplanes[returnval].origin[1]) * normal[1] + (origin[2] - g_mapplanes[returnval].origin[2]) * normal[2];
...
#ifdef HLCSG_CUSTOMCONTENT
		if (assigned && strncasecmp(s->td.name, "content", 7) && strncasecmp(s->td.name, "skip", 4) && contents2 != CONTENTS_ORIGIN && contents2 != CONTENTS_HINT
#ifdef HLCSG_HLBSP_CUSTOMBOUNDINGBOX
